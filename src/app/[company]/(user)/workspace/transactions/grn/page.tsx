...
export default function GRNList() {
  ...
  useEffect(() => {
    const fetchGRNs = async () => {
      ...
      try {
        ...
        if (result.success) {
          // console.log("Fetched GRN:", result.data);
          setData(result.data);
        } else {
  ...
  const filteredData = useMemo(() => {
      return data.filter((p) => {
        const matchSearch =
          `${p.grn_no}  ${p.grn_date} ${p.purchase_no} ${p.supplier_code} ${p.supplier_name}${p.status}`
            .toLowerCase()
        ...
        createPortal(
          <div className="fixed inset-0 z-[99999] bg-black/20 backdrop-blur-sm flex items-center justify-center">
            <div className="bg-white p-8 rounded-xl shadow-2xl flex flex-col items-center gap-3">
              ...
              <p className="text-gray-700 font-semibold text-lg">Loading GRNs...</p>
            </div>
          </div>,
          document.body
        )
        ...
        <table className="ui-table">
          <thead className="ui-table-head">
            <tr className="ui-table-row">
              ...
              <th
                onClick={() => handleSort("grn_date")}><div className="flex items-center gap-1">
                  GRN Date
              ...
              <th
                onClick={() => handleSort("purchase_no")}><div className="flex items-center gap-1">
                  PO No
              ...
              <th
                ...
                <th
                                onClick={() => handleSort("status")}><div className="flex items-center gap-1">
                                  Status
            ...
            {loading ? (
              ...
              paginatedData.map((row) => (
                <tr key={row.id} className="ui-table-row">
                  <td className="ui-table-td">{row.grn_no}</td>
        ...
        <div className="ui-pagination-wrapper">
          <div className="flex flex-col gap-3 w-full">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              ...
              <div className="ui-table-actions">
                <label htmlFor="grn-rows-per-page" className="text-sm text-gray-600">
                  Rows per page
                </label>
                <select
                  id="grn-rows-per-page"
                  value={rowsPerPage}
