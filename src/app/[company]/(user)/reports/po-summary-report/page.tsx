...
export default function POSummarySupplierReport() {
  ...
  const loadReport = async () => {
    ...
    try {
      ...
      const res = await fetch(
        `/api/purchase/po-summary-report?from=${fromDate}&to=${toDate}&supplier=${supplier || ""}`,
  ...
  const totalQty = data.reduce((sum, row) => sum + Number(row.total_qty), 0);
  const pageTotalQty = paginatedData.reduce((sum, row) => sum + Number(row.total_qty), 0);
    ...
    <div className="p-6 max-w-7xl mx-auto space-y-6">
        ...
        createPortal(
          <div className="fixed inset-0 z-[99999] bg-black/20 backdrop-blur-sm flex items-center justify-center">
            <div className="bg-white p-8 rounded-xl shadow-2xl flex flex-col items-center gap-3">
              ...
              <p className="text-gray-700 font-semibold text-lg">Loading report...</p>
            </div>
          </div>,
          document.body
        )}
      ...
      <div className="bg-white p-6 rounded-xl shadow grid md:grid-cols-5 gap-4">
        ...
        <div className="flex items-end">
          <button onClick={loadReport} disabled={loading} className="bg-blue-600 text-white px-5 py-2 rounded-lg hover:bg-blue-700">
          ...
          <table className="w-full text-sm">
            <thead className="bg-indigo-50 text-gray-600 text-sm">
              <tr className="border-t hover:bg-blue-50 transition">
                ...
                <th className="p-3 text-left" onClick={() => handleSort("req_date")}>Req Date</th>
                <th className="p-3 text-left" onClick={() => handleSort("supplier")}>Supplier</th>
                <th className="p-3 text-right" onClick={() => handleSort("total_qty")}>Total Qty</th>
                <th className="p-3 text-left" onClick={() => handleSort("reject_reason")}>Reject Reason</th>
                <th className="p-3 text-left" onClick={() => handleSort("approval_status")}>Status</th>
              </tr>
            </thead>
            <tbody>
                ...
                <tr key={index} className="border-t hover:bg-blue-50 transition">
                  ...
                  <td>
                    <div className="flex flex-col">
                      ...
                      <span className="text-gray-500 text-sm">{row.supplier_code}</span>
                    </div>
                  </td>
                  <td className="text-right pr-4">{Number(row.total_qty)}</td>
