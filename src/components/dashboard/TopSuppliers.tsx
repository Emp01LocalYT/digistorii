...
export default function TopSuppliers() {
    ...
    useEffect(() => {
        const load = async () => {
            ...
            loaded.current = true;
            try {
                const res = await fetch("/api/dashboard/top-suppliers", {
                    headers: {
    ...
    <div className="rounded-2xl border bg-white p-3 shadow-sm">
      <div className="flex justify-between items-center mb-2">
        <div className="flex flex-col">
          <h2 className="text-lg font-bold leading-tight">Suppliers</h2>
                ...
                <div className="flex items-center justify-between mb-1">
                  <div className="flex items-center gap-2">
                    ...
                    <span className={`text-sm font-semibold truncate max-w-[130px] ${
                      ...
                      {item?.supplier_name || "No data"}
                    </span>
                  </div>
                  <span className={`text-xsm font-bold ${item ? "text-gray-900" : "text-gray-200"}`}>
